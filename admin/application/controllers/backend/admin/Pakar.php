...
class Pakar extends AUTH_Controller {
	public function __construct() {
		parent::__construct();
		// $this->load->model('M_pegawai');
		// $this->load->model('M_posisi');
		// $this->load->model('M_kota');
		$perm['userdata'] = $this->userdata;
		if ($perm['userdata']->id_level > 1) {
			echo "
			<script>
			history.back();
			alert('Acces denied');
	...
	public function index() {
		...
		if ($data['userdata']->id_level == 1) {
			$this->template->views('admin/pakar', $data);
		}else{
			echo "string";
		}
	}
